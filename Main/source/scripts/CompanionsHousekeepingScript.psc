...
; 	Debug.Trace("CSQ: Swapping followers.")
	...
	; Here we updated condition to support multiple followers
	if !FollowerScript.CanRecruitFollower()
		...
		if (DBScript.CiceroFollower == 1)
			...
			FollowerScript.SetCiceroFollowing(false)
		endif
		...
		if (DBScript.Initiate1Follower == 1)
			...
			FollowerScript.SetBrotherhoodInitiate1Following(false)
		endif
		...
		if (DBScript.Initiate2Follower == 1)
			...
			FollowerScript.SetBrotherhoodInitiate2Following(false)
		endif

		; If all follower slots are taken and we are not already followed by one of Companions
		; then dismiss first found follower to make room for Companions.
		If !FollowerScript.IsCompanionsFollowing() && !FollowerScript.CanRecruitFollower()
			FollowerScript.DismissFirstFollower(2)
		EndIf
	endif
	; Reserve one slot for Companions
	FollowerScript.SetCompanionsFollowing(true)
EndFunction

Function CleanupFollowerState()
	; Release reserved slot for Companions
	FollowerScript.SetCompanionsFollowing(false)
EndFunction
