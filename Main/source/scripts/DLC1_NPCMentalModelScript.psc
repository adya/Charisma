...
; 	Debug.Trace("RNPC: Locking.")
    ...
    ; See comment in EngageFollowBehavior
    DialogueFollower.SetSeranaFollowing(true)
EndFunction
...
; 	Debug.Trace("RNPC: Unlocking.")
    ...
    ; See comment in EngageFollowBehavior
    DialogueFollower.SetSeranaFollowing(false)
EndFunction

Function EngageFollowBehavior(bool allowDismiss=false)
; 	Debug.Trace("RNPC: Engaging follow.")
    ; Here was a code that prevents the Player from recruiting any followers, 
    ; while quest forced Serana to follow the Player. 
    ; Tecnhically it doesn't make Serana the Player's follower, it only prevents getting another follower.
    ; But with multiple followers support this condition is changed and delegated to DialogueFollower instead.
    ; It will achieve similar result (limiting followers without making Serana an actual follower).
    DialogueFollower.SetSeranaFollowing(true)
...
Function Dismiss()
    if (CanBeDismissed)
        DisengageFollowBehavior()
        IsDismissed = true
        if DialogueFollower.IsFollower(RNPC.GetActorRef())
            DialogueFollower.DismissSpecificFollower(RNPC.GetActorRef())
        endif
        ;RNPC.GetActorReference().SetPlayerTeammate(false)
    endif
EndFunction
